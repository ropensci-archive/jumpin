...
missing_days <- setdiff(as.character(all_days), tools::file_path_sans_ext(dir("CRANlogs"), 
    TRUE))
...
if (rebuild_index) {
    ...
    rm(downloads)
    # If not rebuilding, just load existing index and append data
} else {
...
# Now we reduce the giant CRAN log to just our packages, and just to unique IP
# downloads
