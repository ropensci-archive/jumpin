...
pkgs <- c('alm','AntWeb','bmc','bold','clifro','dependencies','ecoengine','ecoretriever','elastic',
          ...
          'rebird','rentrez','reol','reproducibility-guide','rfigshare','rfishbase','rfisheries',
          'rflybase','rgauges','rgbif','rglobi','rhindawi','rImpactStory','rinat',
...
process_result <- function(x){
  stop_for_status(x)
  if(!x$headers$`content-type` == 'application/json; charset=utf-8') stop('content type mismatch')
  tmp <- content(x, as = 'text')
  jsonlite::fromJSON(tmp, flatten = TRUE)
}

ifemptyelse <- function(x, y){
  if(length(x) == 0) data.frame(NULL) else x[,y]
}
...
gh_check_file <- function(owner='ropensci', repo, ...){
  ...
  if(!req$headers$status == '200 OK'){ NA } else {
...
for_each_pkg <- function(repo){
  ...
  mile <- ifemptyelse(mile, c('id','number','title','open_issues','closed_issues','created_at','due_on','url'))
